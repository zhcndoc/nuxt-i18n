...
`defineI18nLocaleDetector()`{lang="ts"} 组合函数以及示例中使用的 [`@intlify/utils/h3` 工具](https://github.com/intlify/utils#-usage)会自动导入。如果你禁用了自动导入功能，则可以从 `#imports` 中显式导入它们：

```ts [i18n/localeDetector.ts]
import { defineI18nLocaleDetector, tryCookieLocale, tryHeaderLocale, tryQueryLocale } from '#imports'
```

定义语言检测器时，你需要将语言检测器的路径传递给 `experimental.localeDetector` 选项。
