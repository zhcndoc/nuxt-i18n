...
通过使用 `definePageMeta()`{lang="ts"} 设置 `nuxtI18n` 属性来设置动态路由参数的功能已被完全移除，取而代之的是 [`useSetI18nParams()`{lang="ts"} 组合式 API](/docs/composables/use-set-i18n-params)，此功能默认启用，这意味着 `dynamicRouteParams` 选项不再必要。
