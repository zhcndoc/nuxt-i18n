...
使用哪种方法取决于 [`customRoutes` 选项](/docs/api/options#customroutes) 的设置，默认值为 `'page'`{lang="ts-type"}。无法同时使用两种方法。
...
您可以在 `definePageMeta()`{lang="ts"} 中使用 `i18n` 属性为每个页面组件设置自定义路径。

要使用此功能，您需要在 `nuxt.config.ts` 中将 `customRoutes` 设置为 `'meta'`{lang="ts-type"}：
