...
可通过配置文件传递 `createI18n()`{lang="ts"} 的配置。默认情况下，如果未指定任何内容，模块会扫描 `i18n.config{.js,.mjs,.ts}` 文件。
...
- `'meta'`：从页面组件中 `definePageMeta()`{lang="ts"} 函数提取
- `'page'`：从页面组件中 `defineI18nRoute()`{lang="ts"} 宏提取
...
该模块复用 Nuxt 内置的 [`app.cdnURL`](https://nuxt.com/docs/4.x/api/nuxt-config#cdnurl)，通过 CDN / 静态源提供懒加载的语言消息，而不是通过动态 Nitro 服务器提供。这里没有 i18n 专属选项——只需设置 Nuxt 的 `app.cdnURL`，其余由模块自动接管：

- **客户端** 请求目标为 `<app.cdnURL><serverRoutePrefix>/<hash>/<locale>/messages.json`。
- 对应的带哈希的 `messages.json` 文件会在构建时输出到 `.output/public/`，以便与 `_nuxt/*` chunk 一并上传。
- **服务端渲染** 仍从本地 Nitro handler 获取——SSR 不会经由 CDN 往返。
...
2. 将 `.output/public/` 上传到提供 `_nuxt/*` chunks 的同一源站（例如 S3）。
